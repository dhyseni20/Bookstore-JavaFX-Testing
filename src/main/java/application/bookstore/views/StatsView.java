...
public class StatsView extends View {
    ...
    class customPieChart extends PieChart {
        ...
        private void populateChartData(List<Order> orders, ObservableList<Data> pieChartData, dataOptions option) {
            pieChartData.clear();
        ...
        private void addUnit(ObservableList<Data> pieChartData, Unit u) {
            if (u == Unit.$)
        ...
        private void groupByUser(List<Order> orders, ObservableList<Data> pieChartData) {
            for (Order o : orders) {
                boolean match = false;
                ...
                if (!match)
                    pieChartData.add(new Data(o.getUsername(), o.getTotal()));
            }
            addUnit(pieChartData, Unit.$);
        }

        private void groupByClientName(List<Order> orders, ObservableList<Data> pieChartData) {
            for (Order o : orders) {
                boolean match = false;
                for (Data d : pieChartData) {
                ...
                if (!match)
                    pieChartData.add(new Data(o.getClientName(), o.getTotal()));
            }
            addUnit(pieChartData, Unit.$);
        }

        private void incomeGroupByBook(List<Order> orders, ObservableList<Data> pieChartData) {
            class data {
                String title;
                String isbn;
                float total;
        ...
        private void unitsGroupByBook(List<Order> orders, ObservableList<Data> pieChartData) {
            class data {
                String title;
                String isbn;
                int quantity;
