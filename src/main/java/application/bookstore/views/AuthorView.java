...
public class AuthorView extends View {
    ...
    public void deleteAuthors(boolean deleteBooks_) {
        List<Author> itemsToDelete = List.copyOf(getTableView().getSelectionModel().getSelectedItems());
        for (Author a : itemsToDelete) {
            ...
            if (res.matches("1")) {
                ...
                ControllerCommon.showSuccessMessage(getMessageLabel(), "Author removed successfully");
            } else {
                ControllerCommon.showErrorMessage(getMessageLabel(), "Author deletion failed\n" + res);
                break;
            }
        }
    }
